...
@register_model('waitk_transformer_partial_bidirection')
class WaitkTransformerPartialBidirectionModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        encoder = PartialBidirectionTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = TransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return WaitkTransformerPartialBidirectionModel(args, encoder, decoder)


class PartialBidirectionTransformerEncoder(FairseqEncoder):
    """
    ...
    def forward(self, src_tokens, src_lengths=None, mask=None, waitk=None, **kwargs):
        ...
        if mask is None and self.training:
            if waitk is not None:
                ...
                lim = mask.size(0)
                for i in range(0, min(waitk,lim) -1):
                    for j in range(i+1, min(waitk,lim) ):
                        mask[i,j]=0
            else:
                mask = torch.triu(utils.fill_with_neg_inf(x.new(x.size(0), x.size(0))), 1)
...
@register_model_architecture('waitk_transformer_partial_bidirection', 'waitk_transformer_partial_bidirection')
...
@register_model_architecture('waitk_transformer_partial_bidirection', 'waitk_transformer_partial_bidirection_base')
...
@register_model_architecture('waitk_transformer_partial_bidirection', 'waitk_transformer_partial_bidirection_big')
