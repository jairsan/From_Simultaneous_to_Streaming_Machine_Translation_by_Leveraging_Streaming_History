...
from examples.simultaneous_translation.modules import TransformerEncoderLayer, TransformerDecoderLayer


@register_model('waitk_transformer_bidirectional_encoder')
class WaitkTransformerBidirectionalEncoderModel(FairseqEncoderDecoderModel):
    """
    Waitk-Transformer with a bidirectional encoder
    """
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, **kwargs):
        if not self.multi_waitk:
            raise Exception
        
        # At each forward, we randomly decide how many positions are initially waited (k).
        # This defines which encoding positions are available to the decoder at each decoding step i (i+k)
        # Additionally, we will let the first (k) encoding positions to bidirectionally attend between each other (But not to the future)
        # This is because we must define a single encoding for each forward step

        assert self.min_waitk <= self.max_waitk

        if self.training:
            waitk = random.randint(min(self.min_waitk, src_tokens.size()[1]),
                                       min(src_tokens.size()[1], self.max_waitk))

        else:
            waitk = self.waitk

        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, waitk=waitk, **kwargs)
        decoder_out = self.decoder.forward_train(
            prev_output_tokens, waitk=waitk, encoder_out=encoder_out, **kwargs
        )
        return decoder_out
    ...
    def predict_from_states(self, states):
        ...
        x,_=self.decoder.forward(prev_output_tokens=tgt_indices,encoder_out=encoder_out_dict)
    ...
    def predict_from_states_with_probs(self, states):
        self.eval()
        ...
        return token, index[0, 0].item(), self.get_normalized_probs([x[:, -1:]], log_probs=False).max().item()
    ...
    def build_model(cls, args, task):
        ...
        encoder = BidirectionalTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = TransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return WaitkTransformerBidirectionalEncoderModel(args, encoder, decoder)


class BidirectionalTransformerEncoder(FairseqEncoder):
    """
    ...
    def forward(self, src_tokens, src_lengths=None, mask=None, waitk=None, **kwargs):
        """
        ...
        #Bidirectional model, no restrictions to encoder self-attention
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def get_attention_mask(self, x, src_len, waitk):
        if waitk < src_len:
    ...
    def forward_train(self, prev_output_tokens, waitk, encoder_out=None, **kwargs):
...
@register_model_architecture('waitk_transformer_bidirectional_encoder', 'waitk_transformer_bidirectional_encoder')
...
@register_model_architecture('waitk_transformer_bidirectional_encoder', 'waitk_transformer_bidirectional_encoder_base')
...
@register_model_architecture('waitk_transformer_bidirectional_encoder', 'waitk_transformer_bidirectional_encoder_big')
